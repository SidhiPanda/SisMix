...
public class AuthorFilteringStrategy implements FilteringStrategy {
    ...
    public List<JSONObject> getFilteredBooks (String arg, String userId) throws JSONException {
        ...
        for (LibraryBook libraryBook : libraryBooks) {
            List<String> authors = StringUtil.StringToList(libraryBook.getAuthors(), "\\|");
            for (String author : authors) {
                for (String searchParam : searchParams) {
                    if (author.equalsIgnoreCase(searchParam)) {
                        libraryBookSet.add(libraryBook);
                    }
                }
                List<String> subAuthors = StringUtil.StringToList(author, ",");
                for (String subAuthor : subAuthors) {
                    for (String searchParam : searchParams) {
                        if (subAuthor.equalsIgnoreCase(searchParam)) {
                            libraryBookSet.add(libraryBook);
                        }
                    }
                }   
